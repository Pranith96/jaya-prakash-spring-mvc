...
public class UserServiceImpl implements UserService {
	...
	public String resetPassword(String emailId) throws Exception {
		User userResponse = userDao.findUserByEmailId(emailId);
		if (userResponse == null) {
			return "Email id not exists";
		}
		String newPassword = PasswordGenerator.passwordGenerator();
		int response = userDao.updatePassword(userResponse.getEmailId(), newPassword);
		if (response < 0) {
	...
	public String updateUser(UserRequestDto updateUserDto) throws Exception {
		User userResponse = userDao.findUserByEmailId(updateUserDto.getEmailId());
		if (userResponse == null) {
			return "Email id cannot be updated";
		}
		User user = updateUserRequest(updateUserDto, userResponse);
	...
	public User getUser(Integer userId) throws Exception {
		User response = userDao.getUserDetails(userId);
		return response;
	}
